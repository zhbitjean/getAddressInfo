...
# NYC Department of Finance Building Classification code descriptions.
BUILDING_CLASS_DESCRIPTIONS = {'A0': 'CAPE COD',
 ...
 'HS': 'EXTENDED STAY/SUITE: AMENITIES SIMILAR TO APT; TYPICALLY CHARGE WEEKLY RATES & LESS EXPENSIVE THAN '
       'FULL-SERVICE HOTEL',
 ...
 'K2': 'MULTI-STORY RETAIL BUILDING (2 OR \nMORE)',
 ...
 'K7': 'BANKING FACILITIES WITH OR WITHOUT \nPARKING',
 'K8': "BIG BOX \n\tRETAIL: NOT AFFIXED & STANDING ON OWN LOT W/PARKING, E.G. COSTCO & BJ'S",
 ...
 'O7': 'PROFESSIONAL BUILDINGS/STAND \nALONE FUNERAL HOMES',
 ...
 'VG': 'COMMUNITY GARDEN ZONED RESIDENTIAL; NOT MANHATTAN',
...
BUILDING_CLASS_GROUPS = {
    ...
    "U": "Utility Properties",
    "V": "Vacant Land",
    "W": "Educational Facilities",
    "Y": "Government/City Departments",
    "Z": "Miscellaneous Building Classifications",
}
...
_LOWERCASE_WORDS = (" And ", " Or ", " With ", " Without ", " From ", " Of ", " In ", " To ")


def _readable(value: str) -> str:
    text = value.replace(";", " -").replace("APT.", "Apartment").replace("COMML.", "Commercial")
    text = text.replace("RESID.", "Residential").replace("BLDG.", "Building")
    text = text.replace("W/", "with ").replace("W/O", "without ")
    text = text.title()
...
def format_building_class(code: str) -> str:
    value = (code or "").strip().upper()
    if not value:
        return ""
    label = BUILDING_CLASS_OVERRIDES.get(value)
    if label:
        return f"{label} ({value})"
    description = BUILDING_CLASS_DESCRIPTIONS.get(value)
    if not description:
        return value
    group = BUILDING_CLASS_GROUPS.get(value[0], "")
    readable = _readable(description)
    if group and not readable.casefold().startswith(group.casefold()):
        readable = f"{group} - {readable}"
